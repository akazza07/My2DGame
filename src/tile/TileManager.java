...
public class TileManager {
	...
	public void getTileImage() {  // in this we have to add tiles
		    // we don't use the tile 0 to 9 
		    //but I've set a placeholder image so we can prevent NullPointer exception happens when we scan array
		...
		try {
			...
			while(col < gp.maxWorldCol && row < gp.maxWorldRow) {
				...
				while(col < gp.maxWorldCol) {
					...
					mapTileNum[col][row] = num;// we store the extracted number in the maptile Num[][]
					col++;                       
				}
	...
	public void draw(Graphics2D g2) {  // this is little bit complex whole constructor
	...
	//	g2.drawImage(tile[2].image,96, 0, gp.tileSize, gp.tileSize, null);
		...
		while(worldCol < gp.maxWorldCol && worldRow < gp.maxWorldRow) {
			...
			if(worldX + gp.tileSize > gp.player.worldX - gp.player.screenX && 
					// basically this statement for boundary to the screen
